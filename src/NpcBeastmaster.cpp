...
void NpcBeastmaster::ShowMainMenu(Player* player, Creature* creature)
{
    ...
    if (BeastMasterHunterOnly)
    {
        ...
        {
            creature->Whisper("抱歉，宠物只有猎人可用", LANG_UNIVERSAL, player);
            return;
        }
    }
    ...
    {
        std::string messageExperience = Acore::StringFormat("抱歉 {}, 你必须达到 {} 级才能拥有宠物", player->GetName(), BeastMasterMinLevel);
    ...
    // MAIN MENU
    AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "查看宠物", GOSSIP_SENDER_MAIN, PET_PAGE_START_PETS);
    AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "查看罕见宠物", GOSSIP_SENDER_MAIN, PET_PAGE_START_RARE_PETS);
    ...
    if (BeastMasterAllowExotic || player->HasSpell(PET_SPELL_BEAST_MASTERY) || player->HasTalent(PET_SPELL_BEAST_MASTERY, player->GetActiveSpec()))
    {
        if (player->getClass() != CLASS_HUNTER)
        {
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "查看外域宠物", GOSSIP_SENDER_MAIN, PET_PAGE_START_EXOTIC_PETS);
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "查看罕见外域宠物", GOSSIP_SENDER_MAIN, PET_PAGE_START_RARE_EXOTIC_PETS);
        }
    ...
    if (player->getClass() != CLASS_HUNTER && player->HasSpell(PET_SPELL_CALL_PET))
        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "忘却宠物技能", GOSSIP_SENDER_MAIN, PET_REMOVE_SKILLS);

    // Stables for hunters only - Doesn't seem to work for other classes
    if (player->getClass() == CLASS_HUNTER)
        AddGossipItemFor(player, GOSSIP_ICON_TAXI, "打开兽栏", GOSSIP_SENDER_MAIN, GOSSIP_OPTION_STABLEPET);

    // Pet Food Vendor
    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "购买狗粮", GOSSIP_SENDER_MAIN, GOSSIP_OPTION_VENDOR);
...
void NpcBeastmaster::GossipSelect(Player* player, Creature* creature, uint32 action)
{
    ...
    {
        // PETS
        AddGossipItemFor(player, GOSSIP_ICON_TALK, "返回上级..", GOSSIP_SENDER_MAIN, PET_MAIN_MENU);
        int page = action - PET_PAGE_START_PETS + 1;
        int maxPage = pets.size() / PET_PAGE_SIZE + (pets.size() % PET_PAGE_SIZE != 0);

        if (page > 1)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "前一页..", GOSSIP_SENDER_MAIN, PET_PAGE_START_PETS + page - 2);
    ...
    else if (action >= PET_PAGE_START_EXOTIC_PETS && action < PET_PAGE_START_RARE_PETS)
    {
        ...
        if (!(player->HasSpell(PET_SPELL_BEAST_MASTERY) || player->HasTalent(PET_SPELL_BEAST_MASTERY, player->GetActiveSpec())))
        {
            ...
            messageLearn << "我教过你宠物管控的艺术，" << player->GetName() << ".";
        ...
        if (page > 1)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "前一页..", GOSSIP_SENDER_MAIN, PET_PAGE_START_EXOTIC_PETS + page - 2);

        if (page < maxPage)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "后一页..", GOSSIP_SENDER_MAIN, PET_PAGE_START_EXOTIC_PETS + page);
    ...
    else if (action >= PET_PAGE_START_RARE_PETS && action < PET_PAGE_START_RARE_EXOTIC_PETS)
    {
        ...
        if (page > 1)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "前一页..", GOSSIP_SENDER_MAIN, PET_PAGE_START_RARE_PETS + page - 2);

        if (page < maxPage)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "后一页..", GOSSIP_SENDER_MAIN, PET_PAGE_START_RARE_PETS + page);
    ...
    else if (action >= PET_PAGE_START_RARE_EXOTIC_PETS && action < PET_PAGE_MAX)
    {
        ...
        if (page > 1)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "前一页..", GOSSIP_SENDER_MAIN, PET_PAGE_START_RARE_EXOTIC_PETS + page - 2);

        if (page < maxPage)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "后一页..", GOSSIP_SENDER_MAIN, PET_PAGE_START_RARE_EXOTIC_PETS + page);
...
void NpcBeastmaster::CreatePet(Player* player, Creature* creature, uint32 action)
{
    ...
    {
        creature->Whisper("首先你必须放弃或拴好现在的宠物", LANG_UNIVERSAL, player);
        CloseGossipMenuFor(player);
        return;
    }
    ...
    // Farewell
    std::string messageAdopt = Acore::StringFormat("明智的选择 {} ！好好对待你的宠物 {} ，这样你永远不会一个人面对敌人。", player->GetName(), pet->GetName());
